...
```
6시간 간격 (UTC 00:00/06:00/12:00/18:00) Supabase Edge Function 실행
  → DB에서 기한 초과 마일스톤 일괄 조회
  → status 'past_due'로 일괄 업데이트 (멱등성 보장)
  → 프로젝트별 Webhook 알림 발송 (첫 감지 시에만)
  → 실행 로그 기록
```
...
### 확정 방안: Supabase Edge Function + Vercel Cron (6시간 간격, 4회/일)

> **CEO 확정 (2026-04-02):** 글로벌 사용자 커버리지를 위해 UTC 기준 6시간 간격 4회 실행.
> 단일 KST 09:00 → UTC 00:00/06:00/12:00/18:00 변경.

Supabase 무료/Pro 플랜에서는 pg_cron을 직접 사용할 수 없으므로, **Edge Function + Vercel Cron**을 조합합니다.

```
┌──────────────────────────────────────────────────────────────┐
│          UTC 00:00 / 06:00 / 12:00 / 18:00 (4회/일)          │
│          → KST 09:00 / 15:00 / 21:00 / 03:00                │
│          → EU  01:00 / 07:00 / 13:00 / 19:00 (CET)          │
│          → US  19:00 / 01:00 / 07:00 / 13:00 (EST)          │
│                                                               │
│  ┌──────────────┐     ┌──────────────────────────────┐       │
│  │ Vercel Cron   │────▶│ Edge Function                 │      │
│  │ 0 0,6,12,18  │     │ check-milestone-past-due      │      │
│  │  * * *        │     │                               │      │
│  └──────────────┘     │ 1. 기한 초과 milestone 조회    │      │
│                       │ 2. status → past_due 업데이트  │      │
│                       │ 3. 알림 대상 project 집계      │      │
│                       │ 4. Webhook 발송 (Slack/Teams)  │      │
│                       │ 5. 실행 로그 기록              │      │
│                       └──────────────────────────────┘       │
└──────────────────────────────────────────────────────────────┘
```

**멱등성 보장:** `status NOT IN ('past_due', 'completed')` 조건으로 4회 실행해도 중복 알림 없음.
...
**→ UTC 00:00 / 06:00 / 12:00 / 18:00 (6시간 간격, 4회/일)**
**→ 글로벌 타임존 커버: KST 09:00, CET 07:00, EST 07:00 각각 근접**
...
| 3 | 하루 4회 실행 (6시간 간격) | `status NOT IN ('past_due', 'completed')` 조건으로 멱등성 보장, 중복 알림 없음 |
