...
| 패키지 | 상태 | 역할 |
|-------|-----|------|
| `@testably.kr/reporter-core` | Stable (1.0.1) | 공통 HTTP 클라이언트 (reporter 공유 코어) |
| `@testably.kr/playwright-reporter` | Stable (1.0.1) | Playwright → Testably 결과 자동 업로드 |
| `@testably.kr/cypress-reporter` | Stable (1.0.1) | Cypress → Testably 결과 자동 업로드 (`after:run`) |
| `@testably.kr/jest-reporter` | Planned | Jest → Testably 결과 자동 업로드 (향후 공식 지원 예정) |
