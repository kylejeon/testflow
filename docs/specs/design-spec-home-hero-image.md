...
> **중요 (2026-04-24 개정):** 실제 Testably 앱은 **라이트 테마 단일**이다 (`src/pages/plan-detail/page.tsx` L1951 `background: '#fff'`, `src/lib/environments.ts` L84 `HEATMAP_COLORS`). 마케팅 홈은 다크 테마이므로 히어로 안에 라이트 스크린샷이 들어가면 "다크 배경 + 라이트 앱 샷" 콘트라스트(Linear / Notion / Figma 패턴)가 자연스럽게 형성된다. 이전 개정(다크 버전)은 폐기.

### 4-1. 히어로 메인 이미지 (Light — 제품과 일치)

| 요소 | 토큰 | Hex |
|---|---|---|
| 이미지 전체 배경 (앱 캔버스) | `bg-gray-50` | `#F9FAFB` |
| ProjectHeader 배경 | `bg-white` + bottom border `gray-200` | `#FFFFFF` / `#E5E7EB` |
| Breadcrumb text | `text-gray-500` (활성 조각은 `text-gray-700`) | `#6B7280` / `#374151` |
| Plan H1 | `text-gray-900` | `#0F172A` |
| Active 배지 | `bg-amber-100 text-amber-800 border-amber-300` | bg `#FEF3C7`, fg `#92400E` |
| P1 배지 | `bg-red-100 text-red-700 border-red-300` | bg `#FEE2E2`, fg `#B91C1C` |
| 탭 inactive | `text-gray-500` | `#6B7280` |
| 탭 active (Environments) | `text-indigo-600` + `border-b-2 border-indigo-500` | fg `#4F46E5`, border `#6366F1` |
| Coverage Matrix 카드 | `bg-white border border-gray-200` | `#FFFFFF` / `#E5E7EB` |
| Matrix 헤더 "COVERAGE MATRIX" | `text-indigo-700 uppercase tracking-wider` | `#4338CA` |
| OS group 헤더 배경 | `bg-gray-50` | `#F9FAFB` |
| **히트맵 셀 (코드 `HEATMAP_COLORS`와 1:1 일치)** | — | — |
| └ `perfect` (95–100%) | bg `#DCFCE7` fg `#14532D` | green-100 / green-900 |
| └ `pass` (75–95%) | bg `#86EFAC` fg `#14532D` | green-300 / green-900 |
| └ `mixed` (60–75%) | bg `#FDE68A` fg `#78350F` | amber-200 / amber-900 |
| └ `warn` (40–60%) | bg `#FCA5A5` fg `#7F1D1D` | red-300 / red-900 |
| └ `fail` (20–40%) | bg `#EF4444` fg `#FFFFFF` | red-500 / white |
| └ `critical` (0–20%) | bg `#991B1B` fg `#FFFFFF` | red-800 / white — **라이트 배경 대비 극대** |
| └ `untested` | bg `#FAFAFA` dashed border `#E5E7EB` fg `#9CA3AF` | gray-50 / gray-200 / gray-400 |
| AI Insights 카드 컨테이너 | `bg-white border border-violet-100` | `#FFFFFF` / `#EDE9FE` |
| AI Insights 상단 그라데이션 밴드 | `from-violet-100/70 to-indigo-100/30` | subtle |
| AI 배지 "AI-GENERATED" | `text-violet-700` uppercase tracking-widest | `#6D28D9` |
| Critical 카드 | `bg-red-50 border-red-200 text-red-900` | `#FEF2F2` / `#FECACA` / `#991B1B` |
| Gap 카드 | `bg-amber-50 border-amber-200 text-amber-900` | `#FFFBEB` / `#FDE68A` / `#78350F` |
| Baseline 카드 | `bg-green-50 border-green-200 text-green-900` | `#F0FDF4` / `#BBF7D0` / `#14532D` |
| AI 액션 chip (보라) | `bg-violet-50 border-violet-200 text-violet-700` | `#F5F3FF` / `#DDD6FE` / `#6D28D9` |
| **플로팅 Run Summary** | — | — |
| └ 헤더 (브랜드 AI 액센트로 다크 유지) | `linear-gradient(135deg,#4F46E5,#312E81)` | indigo-600 → indigo-900 |
| └ 헤더 타이틀 | `text-white` | `#FFFFFF` |
| └ 바디 | `bg-white border border-indigo-200` | `#FFFFFF` / `#C7D2FE` |
| └ LOW RISK 배지 | `bg-green-100 text-green-900 border-green-300` | `#DCFCE7` / `#14532D` / `#86EFAC` |
| └ Metric 카드 | `bg-slate-50 border-gray-200` | `#F8FAFC` / `#E5E7EB` |
| └ 숫자: TOTAL / PASSED / FAILED / BLOCKED / RATE / SKIPPED | 각각 slate-900 · green-600 · red-600 · amber-600 · yellow-700 · slate-500 | `#0F172A` / `#16A34A` / `#DC2626` / `#D97706` / `#CA8A04` / `#64748B` |
| └ 외곽 글로우 | indigo radial, opacity 0.28 | `#6366F1` |

### 4-2. og:image (1200 × 630)

og:image는 Slack / Twitter / LinkedIn 같은 라이트 테마 UI에서 썸네일로 뜬다. 본 히어로는 라이트 테마이므로 SNS 미리보기에서도 자연스럽게 읽히되, **좌측 30%에 다크 그라디언트 오버레이**를 얹어 "Testably" 워드마크와 H1이 명료하게 대비되도록 한다 (§7 참조).
