 ...
 *   - three-col: Testably vs A vs B (reserved for M3 vs-matrix)
 *
 * Visual: same JSX pattern that lives inline in src/pages/compare/page.tsx.
 * Extracted so /alternatives/{slug} can reuse without dragging in the entire
 * /compare/{slug} page shell.
 */
...
export interface ComparisonTableProps {
  ...
  /** Column headers */
  headers: {
    feature: string; // "Feature" or "Plan"
    testably: string; // "Testably"
    competitor: string; // competitor name
  };
  rows: TwoColRow[];
  /** Renderer for pricing rows where each cell holds { price, detail } */
  renderCell?: (
    value: boolean | string,
    isTestably: boolean,
  ) => ReactNode;
  /** Caption rendered below the table (e.g. "Last updated: May 2026") */
  caption?: string;
}
