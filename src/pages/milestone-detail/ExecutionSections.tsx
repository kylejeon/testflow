...
/**
 * Design-spec §5 — Execution sections.
 */
export default function ExecutionSections({
  ...
  const allEmpty = subMilestones.length === 0 && plans.length === 0 && runs.length === 0 && sessions.length === 0;

  // AC-C11: unified empty state when everything is 0
  if (allEmpty && !plansLoading) {
    return (
      <div className="mo-empty">
        <i className="ri-folder-open-line mo-empty-icon" style={{ fontSize: 32, display: 'block' }} />
        <div style={{ fontWeight: 600, color: 'var(--text)' }}>No runs or plans yet.</div>
        <div style={{ marginTop: 4 }}>Create a plan to start tracking executions.</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      {plansLoading ? (
        ...
        <div className="mo-empty">
          <i className="ri-folder-chart-line mo-empty-icon" style={{ fontSize: 32, display: 'block' }} />
          <div>No test plans linked to this milestone</div>
        </div>
      ) : (
        <div className="mo-sec-card">
          {plans.map(plan => (
      ...
      {runs.length === 0 ? (
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 10, padding: 24, textAlign: 'center', color: 'var(--text-subtle)', fontSize: 13 }}>
          No runs yet
        </div>
      ) : (
        <div className="mo-sec-card">
